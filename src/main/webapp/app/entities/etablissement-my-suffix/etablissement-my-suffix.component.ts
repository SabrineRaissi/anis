...
export class EtablissementMySuffixComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.etablissementService
            ...
            .subscribe(
                (res: HttpResponse<IEtablissementMySuffix[]>) => this.paginateEtablissements(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/etablissement-my-suffix'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: IEtablissementMySuffix) {
        return item.id;
    }

    registerChangeInEtablissements() {
        this.eventSubscriber = this.eventManager.subscribe('etablissementListModification', response => this.loadAll());
    }
    ...
    protected paginateEtablissements(data: IEtablissementMySuffix[], headers: HttpHeaders) {
